✦ Mandau Installation and Setup Guide
  ...
  Production Deployment with Custom Hostnames:

   1 # Set environment variables for production
   2 export MANDAU_CORE_HOSTNAME=mandau-core.example.com
   3 export MANDAU_CORE_IP=192.168.1.100
   4 export MANDAU_AGENT_HOSTNAME=agent-us-east-1
   5 export MANDAU_AGENT_IP=192.168.1.101
   6
   7 # Install with production certificates
   8 curl -fsSL https://raw.githubusercontent.com/bhangun/mandau/main/scripts/install.sh | sudo bash

  Note: For multi-server deployments, use cert-distribute.sh instead. See Certificate Management Guide for details.
  ...
  **Development Setup (Single Machine):**
   ...
   5 git clone https://github.com/bhangun/mandau.git
   6 cd mandau
   7 make certs
   8 cp -r certs ~/mandau-certs/

  **Production Setup (Multiple Servers):**

  For production, you should use centralized CA management. See the [Certificate Management Guide](CERTIFICATE_MANAGEMENT.md) for complete instructions.

  Quick Production Deployment:

   1 # Generate CA on secure admin machine (do this ONCE)
   2 mkdir -p ~/mandau-ca
   3 cd ~/mandau-ca
   4 openssl genrsa -out ca.key 4096
   5 openssl req -new -x509 -days 3650 -key ca.key -out ca.crt -subj "/CN=Mandau CA/O=Mandau/C=US"
   6 chmod 600 ca.key
   7
   8 # Distribute to servers using cert-distribute.sh
   9 cd ~/mandau  # Where you cloned Mandau
  10 ./scripts/cert-distribute.sh --type core --host 192.168.1.100 --agent-hostname mandau-core.example.com --agent-ip 192.168.1.100
  11 ./scripts/cert-distribute.sh --type agent --host 192.168.1.101 --agent-hostname agent-us-east-1 --agent-ip 192.168.1.101
  ...
  Mandau uses a PKI (Public Key Infrastructure) system:
   ...
   - CA Certificate (ca.crt): Signs all other certificates (keep secure!)
   - Core Certificate (core.crt + core.key): For the Core service
   - Agent Certificate (agent.crt + agent.key): For each Agent (unique per server)
   - Client Certificate (client.crt + client.key): For CLI clients

  ⚠️ Production Best Practices:

   - Generate CA ONCE on a secure admin machine
   - Distribute CA certificate (ca.crt) to all servers
   - NEVER distribute CA key (ca.key) to servers
   - Generate unique agent certificates per server
   - Set correct SANs (hostname + IP) for each server

  For complete certificate management, see the [Certificate Management Guide](CERTIFICATE_MANAGEMENT.md).
